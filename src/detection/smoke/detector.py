"""Smoke detector plugin implementation for AERA.

This module defines the SmokeDetector class which inherits from BaseDetector
and performs smoke detection inference using a HeuristicSmokeModel.
"""
...
class SmokeDetector(BaseDetector):
    """Smoke detector plugin analyzing video frames for smoke presence."""

    def __init__(
        self,
        model_path: str = "models/smoke_yolov8.pt",
        confidence_threshold: float = 0.5,
        model_loader: Optional[ModelLoader] = None,
    ) -> None:
        """Initialize the SmokeDetector.

        Args:
            model_path: Path to the smoke detection model weights.
    ...
    def load_model(self) -> None:
        """Load the smoke detection model weights via the ModelLoader.
        ...
        try:
            ...
            # Use model_loader to load/cache the HeuristicSmokeModel
            # TODO: In future versions, this can be swapped with a real YOLOv8/ONNX model loader hook.
            self.model = self.model_loader.load_model(
                model_path=self.model_path,
                model_type="smoke",
                load_fn=lambda path: HeuristicSmokeModel(path),
            )
        except Exception as e:
            raise ModelLoadError(f"Failed to load smoke detection model: {e}") from e

    def detect(self, frame: np.ndarray) -> List[DetectionResult]:
        """Perform smoke detection inference on the frame.
        ...
        try:
            # Delegate inference to the SmokeModel
            ...
            raise InferenceError(f"Inference failed in SmokeDetector: {e}") from e
