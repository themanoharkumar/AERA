...
class BackendGateway:
    ...
    def __init__(self) -> None:
        ...
        self.camera_service = CameraService(self.camera_repository, self.coordinator.camera_manager)
    ...
    def _register_default_cameras(self) -> None:
        """Register the persisted cameras. Seeds default ones on first run."""
        persisted = self.camera_repository.list_cameras()
        if not persisted:
            logger.info("Camera database repository is empty. Seeding defaults...")
            defaults = [
                {"camera_id": "webcam_0", "name": "Webcam Feed", "type": "WEBCAM", "source": "0", "location": "Security Room", "description": "Local USB webcam", "enabled": True},
                {"camera_id": "local_video_1", "name": "Corridor File", "type": "FILE", "source": "dummy_source_1", "location": "Corridor A", "description": "Corridor loop video file", "enabled": True},
                {"camera_id": "local_video_2", "name": "Lobby File", "type": "FILE", "source": "dummy_source_2", "location": "Central Lobby", "description": "Lobby simulation file", "enabled": True},
            ]
            for item in defaults:
                self.camera_service.register_camera(item)
            persisted = self.camera_repository.list_cameras()

        for cam in persisted:
            if cam["enabled"]:
                try:
                    ...
                    self.coordinator.camera_manager.start_camera(cam["camera_id"])
                    logger.info("Loaded and started persisted camera stream: %s (ID: %s)", cam["name"], cam["camera_id"])
                except Exception as e:
                    logger.warning("Persisted camera '%s' failed to start on init: %s", cam["camera_id"], e)
...
_global_gateway: Optional[BackendGateway] = None
_gateway_lock = threading.Lock()


def get_backend_gateway() -> BackendGateway:
    """Access or initialize the thread-safe gateway as a global process-wide singleton."""
    global _global_gateway
    with _gateway_lock:
        if _global_gateway is None:
            _global_gateway = BackendGateway()
