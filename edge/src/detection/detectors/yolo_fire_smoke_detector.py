...
class UltralyticsYOLOModel(BaseModel):
    ...
    def __init__(self, model_path: str) -> None:
        ...
        # Resolve model path if relative and not found in current working directory
        resolved_path = model_path
        if not os.path.isabs(resolved_path) and not os.path.exists(resolved_path):
            alt_path = os.path.join("edge", model_path)
        ...
        try:
            ...
            logger.info("YOLO Model Init: Loading weights from '%s' on device '%s'", resolved_path, self.device)
